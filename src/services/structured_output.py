...
class WhatsAppListItem(BaseModel):
    ...
    description: Optional[str] = Field(None, max_length=72, description="Item description")
...
class WhatsAppListSection(BaseModel):
    ...
    items: List[WhatsAppListItem] = Field(..., min_items=1, description="Items in this section")
            ...
            "sections": [
                {
                    ...
                    "rows": [
                        ...
                        for item in self.projects[:10]  # WhatsApp limit: 10 items per section
                    ]
                }
            ]
        }
...
class ActionButtonsOutput(BaseModel):
    ...
    intro_text: str = Field(..., description="Introduction message")
    buttons: List[WhatsAppButton] = Field(..., max_items=3, description="Action buttons (max 3)")
